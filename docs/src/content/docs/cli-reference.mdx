...
| `clawq delegate [--runner auto|kimi|opencode|codex|claude|gemini|cursor] [--repo PATH] [--branch NAME] GOAL` | High-level shortcut for handing off a background coding task with sensible defaults; auto order: kimi → cursor → opencode (zai-coding-plan/glm-5) → codex → claude → gemini |
