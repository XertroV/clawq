...
function toothAngle(teeth: number): number {
...
function deriveRenderableRadii(pitchRadius: number, circularPitch: number) {
  ...
  const outerRadius = pitchRadius + toothAmplitude;
  const rootRadius = Math.max(pitchRadius - toothAmplitude, pitchRadius * 0.7);
  return {
    module: gearModule,
    outerRadius,
    rootRadius,
...
export function solveGearScene(scene: GearScene): SolvedGearScene {
  ...
  if (driverChildren.length > 0) {
    driver.phaseDeg = toDegrees(driverChildren[0].placement!.angleRad - toothAngle(driver.teeth) * 0.5);
  }
  ...
  for (const gear of scene.gears) {
    ...
    current.phaseDeg = toDegrees(contactAngle);
  }
