...
let wait_for_rate_limit ~route =
  ...
  let* () =
    if !global_rate_limit > now then
      Lwt_unix.sleep (!global_rate_limit -. now)
...
let update_rate_limit ~route ~headers =
  let remaining =
    try Cohttp.Header.get headers "x-ratelimit-remaining"
        |> Option.map int_of_string
    with _ -> None
  in
  let reset_at =
    try Cohttp.Header.get headers "x-ratelimit-reset"
        |> Option.map float_of_string
    with _ -> None
  in
  let is_global =
    try Cohttp.Header.get headers "x-ratelimit-global"
  ...
  if is_global then begin
    let retry_after =
      try Cohttp.Header.get headers "retry-after"
          |> Option.map float_of_string
  ...
  | Some r, Some ra ->
    let bucket = match Hashtbl.find_opt route_buckets route with
      | Some b -> b
      | None ->
  ...
  Lwt_mutex.with_lock route_mutex (fun () ->
    ...
    if status = 429 then begin
      let retry_after =
        ...
        with _ -> 1.0
      in
      Logs.warn (fun m -> m "Discord: rate limited on %s, retrying after %.1fs" route retry_after);
...
let is_fatal_close_code code =
  match code with
  | 4004 | 4010 | 4011 | 4012 | 4013 | 4014 -> true
  | _ -> false
...
let start ~config ~session_manager =
  ...
  | Some discord_config ->
    if discord_config.bot_token = "" then begin
      ...
      Logs.info (fun m -> m "Discord channel starting (WebSocket gateway mode)");
      let open Lwt.Syntax in
      ...
      let rec connect_loop () =
        ...
        let on_dispatch event_name d =
          if event_name = "MESSAGE_CREATE" then
            ...
            | Some msg ->
              handle_message ~discord_config ~session_mgr:session_manager msg
            | None -> Lwt.return_unit
          else
            Lwt.return_unit
        in
        let on_close code =
          if Lwt.is_sleeping close_p then
            Lwt.wakeup_later close_u code;
          Lwt.return_unit
        in
        let result =
          Lwt.catch
        ...
        | Ok code ->
          ...
          if is_fatal_close_code code_int then begin
            Logs.err (fun m -> m "Discord: fatal close code %d, not reconnecting" code_int);
            Lwt.return_unit
          end else begin
            Logs.info (fun m -> m "Discord: connection closed (code=%d), reconnecting"
              code_int);
            let delay = if !resume_session_id <> None then 1.0 else !backoff in
            backoff := (if !resume_session_id = None then Float.min (!backoff *. 2.0) 60.0 else 1.0);
            let* () = Lwt_unix.sleep delay in
            connect_loop ()
          end
      in
