...
let start_heartbeat t =
  (match t.heartbeat_stop with
   | Some u -> Lwt.wakeup_later u (); t.heartbeat_stop <- None
   | None -> ());
  ...
  Lwt.async (fun () ->
    ...
    if not (Lwt.is_sleeping stop_p) then Lwt.return_unit
      ...
      let rec loop () =
        ...
        let* () = Lwt.pick [
          Lwt_unix.sleep (t.heartbeat_interval /. 1000.0);
          stop_p;
        ] in
        if not (Lwt.is_sleeping stop_p) then Lwt.return_unit
        else if not t.heartbeat_ack_received then begin
          Logs.warn (fun m -> m "Discord: heartbeat ACK not received, zombie connection");
...
let handle_gateway_message t msg =
  ...
  | json ->
    ...
    | 10 -> (* Hello *)
      ...
      start_heartbeat t;
      Logs.info (fun m -> m "Discord: Hello received, heartbeat_interval=%.0fms" interval);
    ...
    | 0 -> (* Dispatch *)
      ...
      if event_name = "READY" then begin
        let sid = try Some (d |> member "session_id" |> to_string) with _ -> None in
        let rurl = try Some (d |> member "resume_gateway_url" |> to_string) with _ -> None in
        t.session_id <- sid;
        t.resume_gateway_url <- rurl;
        Logs.info (fun m -> m "Discord: READY, session_id=%s"
    ...
    | 9 -> (* Invalid Session *)
      let resumable =
        try json |> member "d" |> to_bool with _ -> false
      in
      if resumable then begin
        Logs.info (fun m -> m "Discord: Invalid Session (resumable), waiting before resume");
        let* () = Lwt_unix.sleep (1.0 +. Random.float 4.0) in
        send_resume t
      end else begin
        Logs.info (fun m -> m "Discord: Invalid Session (not resumable), re-identifying");
        t.session_id <- None;
        t.seq <- None;
        send_identify t
      end
...
let get_gateway_url ~bot_token =
  ...
  if status >= 200 && status < 300 then
    ...
    with _ ->
      Lwt.fail_with "Discord: failed to parse gateway/bot response"
  else
    Lwt.fail_with (Printf.sprintf "Discord: gateway/bot returned HTTP %d: %s" status body)
...
let connect ~bot_token ~intents ?resume_session_id ?resume_seq ?resume_url
  ...
  in
  Logs.info (fun m -> m "Discord: connecting to gateway %s"
