...
let is_shell_safe s = negb (string_has_metachar s)
...
let is_whitespace c = c = char_space || c = char_tab

(** val is_quote_char : char -> bool **)

let is_quote_char c = c = char_sq || c = char_dq
...
let rec string_of_chars = function
  ...
  | c :: rest ->
      (* If this appears, you're using String internals. Please don't *)
      (fun (c, s) -> String.make 1 c ^ s) (c, string_of_chars rest)

type quote_state = NoQuote | InQuote of char

(** val flush_word : char list -> string list -> string list **)

let flush_word cur words =
  match cur with [] -> words | _ :: _ -> string_of_chars cur :: words

(** val parse_chars : char list -> char list -> string list -> quote_state ->
    string list option **)
...
let rec parse_chars chars cur words q =
  ...
  | c :: rest -> (
      ...
      | NoQuote ->
          ...
          else if is_quote_char c then parse_chars rest cur words (InQuote c)
          else if c = char_backslash then
            match rest with
            | [] -> Some (rev (flush_word (app cur (c :: [])) words))
            | next :: rest' ->
                parse_chars rest' (app cur (next :: [])) words NoQuote
          else parse_chars rest (app cur (c :: [])) words NoQuote
      | InQuote q_char ->
          if c = q_char then parse_chars rest cur words NoQuote
          else if c = char_backslash && q_char = char_dq then
            match rest with
            | [] -> None
            | next :: rest' ->
                parse_chars rest' (app cur (next :: [])) words (InQuote q_char)
          else parse_chars rest (app cur (c :: [])) words (InQuote q_char))

(** val split_words : string -> string list option **)

let split_words s = parse_chars (list_of_string s) [] [] NoQuote

(** val is_allowed : string -> string list -> bool **)

let is_allowed cmd allowlist = existsb (( = ) cmd) allowlist
