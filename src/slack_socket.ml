  ...
  if status >= 200 && status < 300 then
    try
      ...
      if ok then
        ...
        let err = try json |> member "error" |> to_string with _ -> "unknown" in
        Lwt.fail_with ("Slack Socket Mode: apps.connections.open error: " ^ err)
    with exn ->
      Lwt.fail_with ("Slack Socket Mode: failed to parse response: " ^ Printexc.to_string exn)
  else
    Lwt.fail_with (Printf.sprintf "Slack Socket Mode: HTTP %d from apps.connections.open" status)
...
let run_connection ~(config : Runtime_config.slack_config)
  ...
  Ws_client.on_message ws (fun msg ->
    ...
    | None ->
      Logs.debug (fun m -> m "Slack Socket Mode: non-envelope message ignored");
      Lwt.return_unit
    | Some env ->
      ...
      | "disconnect" ->
        ...
        Ws_client.close ws;
        if Lwt.is_sleeping done_p then
      ...
      | "events_api" ->
        ...
        Lwt.return_unit
      | other ->
        Logs.debug (fun m -> m "Slack Socket Mode: unhandled type %s" other);
        Lwt.return_unit);
  let* () = Lwt.pick [
    done_p;
    Ws_client.closed ws;
...
let start ~config ~session_manager =
  ...
  | Some sc ->
    ...
    let rec loop () =
      ...
      (match outcome with
       | Ok () ->
         Logs.info (fun m -> m "Slack Socket Mode: connection closed, reconnecting in 1s");
       ...
       | Error err ->
         ...
         Logs.info (fun m -> m "Slack Socket Mode: reconnecting in %.0fs" delay);
