module Ws_tls =
  Httpun_ws_lwt.Client (Gluten_lwt_unix.Client.TLS)
...
let connect_wss ~uri () =
  ...
  let resource =
    if query = "" then (if path = "" then "/" else path)
    else path ^ "?" ^ query
  in
  let nonce =
    Mirage_crypto_rng.generate 16 |> Base64.encode_exn
  in
  let* addrs =
  ...
  let resolve_closed () =
    if not (is_resolved closed_p) then
      Lwt.wakeup_later closed_u ()
  in
            ...
            (match opcode with
             ...
             | `Connection_close ->
               ...
               if String.length payload_str >= 2 then begin
                 let code = Char.code payload_str.[0] lsl 8 lor Char.code payload_str.[1] in
  ...
  in
  let _conn =
    Ws_tls.connect ~nonce ~host ~port ~resource
      ~error_handler ~websocket_handler:websocket_handler' tls_socket
  in
  let* wsd_result = Lwt.pick [
    (let* w = wsd_p in Lwt.return (Ok w));
    (let* () = closed_p in Lwt.return (Error "WebSocket connection closed before handshake completed"));
  ] in
...
let close t =
  if not (Httpun_ws.Wsd.is_closed t.wsd) then
    Httpun_ws.Wsd.close t.wsd

let on_message t cb =
  t.on_message_cb <- Some cb
