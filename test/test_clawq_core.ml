let test_parse_agent () =
  Alcotest.(check string) "parse agent"
    "CmdAgent"
...
let test_parse_unknown () =
  Alcotest.(check string) "parse garbage"
    "CmdUnknown"
...
let test_dispatch_empty () =
  ...
  Alcotest.(check bool) "dispatch [] contains Usage"
    true (String.length result > 0 && String.sub result 0 5 = "Usage")

let test_dispatch_version () =
  Alcotest.(check string) "dispatch version"
    "clawq 0.1.0-dev"
    (Clawq_core.dispatch [ "version" ])

let test_dispatch_help () =
  let result = Clawq_core.dispatch [ "help" ] in
  Alcotest.(check bool) "dispatch help contains Usage"
    true (String.length result > 0 && String.sub result 0 5 = "Usage")
