let test_handle_hello () =
  ...
  let gw : Discord_gateway.t = {
    ...
    intents = 513;
    on_dispatch = (fun name d -> dispatches := (name, d) :: !dispatches; Lwt.return_unit);
  ...
  Alcotest.(check (float 0.1)) "heartbeat_interval" 41250.0 gw.heartbeat_interval;
...
let test_handle_dispatch_ready () =
  ...
  let ready_msg = {|{"op":0,"s":1,"t":"READY","d":{"session_id":"abc123","resume_gateway_url":"wss://resume.example.com"}}|} in
  ...
  Alcotest.(check (option string)) "resume_url" (Some "wss://resume.example.com") gw.resume_gateway_url;
...
let test_handle_dispatch_message_create () =
  ...
  let msg = {|{"op":0,"s":6,"t":"MESSAGE_CREATE","d":{"channel_id":"ch1","author":{"id":"u1","bot":false},"content":"hello"}}|} in
...
let test_handle_invalid_session_resumable () =
  ...
  Alcotest.(check (option string)) "session_id preserved" (Some "sess") gw.session_id;
...
let test_handle_invalid_session_not_resumable () =
  ...
  Lwt_main.run (Discord_gateway.handle_gateway_message gw {|{"op":9,"d":false}|});
...
let test_seq_updates () =
  ...
  Lwt_main.run (Discord_gateway.handle_gateway_message gw
...
let test_hello_with_existing_session_triggers_resume () =
  ...
  Alcotest.(check (option string)) "session_id preserved" (Some "existing-session") gw.session_id;
...
let test_hello_integer_heartbeat_interval () =
  ...
  Lwt_main.run (Discord_gateway.handle_gateway_message gw hello_msg);
  Alcotest.(check (float 0.1)) "heartbeat_interval from int" 45000.0 gw.heartbeat_interval;
  (match gw.heartbeat_stop with Some u -> Lwt.wakeup_later u () | None -> ())
...
let test_accessor_functions () =
  ...
  Alcotest.(check (option string)) "session_id" (Some "sid1") (Discord_gateway.session_id gw);
  Alcotest.(check (option int)) "last_seq" (Some 99) (Discord_gateway.last_seq gw);
  Alcotest.(check (option string)) "resume_url" (Some "wss://resume.example.com") (Discord_gateway.resume_url gw)
...
let suite =
  [
    ...
    Alcotest.test_case "handle dispatch message_create" `Quick test_handle_dispatch_message_create;
    Alcotest.test_case "handle invalid session resumable" `Quick test_handle_invalid_session_resumable;
    Alcotest.test_case "handle invalid session not resumable" `Quick test_handle_invalid_session_not_resumable;
    Alcotest.test_case "handle heartbeat request" `Quick test_handle_heartbeat_request;
    ...
    Alcotest.test_case "hello with session triggers resume" `Quick test_hello_with_existing_session_triggers_resume;
    Alcotest.test_case "dispatch no seq field" `Quick test_dispatch_no_seq_field;
    Alcotest.test_case "hello integer heartbeat interval" `Quick test_hello_integer_heartbeat_interval;
