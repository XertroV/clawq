...
let test_init_schema_version_is_14 () =
  let db = Memory.init ~db_path:":memory:" () in
  Alcotest.(check int)
    "schema version is 14" 14
...
let test_init_rejects_future_schema_version () =
  with_temp_db (fun db_path ->
      let db = Sqlite3.db_open db_path in
      exec_exn db "CREATE TABLE schema_version (version INTEGER NOT NULL)";
      exec_exn db "INSERT INTO schema_version (version) VALUES (15)";
      exec_exn db
        {|CREATE TABLE messages (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  session_key TEXT NOT NULL,
  ...
  created_at TEXT NOT NULL DEFAULT (datetime('now'))
      ...
      | `Msg msg ->
          Alcotest.(check bool)
            ...
            (String.starts_with ~prefix:"DB uses future schema version 15" msg))
...
let suite =
  [
    ...
    Alcotest.test_case "init schema version is 14" `Quick
      test_init_schema_version_is_14;
