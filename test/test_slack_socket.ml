let test_parse_envelope_valid () =
  let msg = {|{"envelope_id":"env123","type":"events_api","payload":{"event":{"type":"message","channel":"C1","user":"U1","text":"hi"}}}|} in
...
let test_parse_envelope_no_id () =
  ...
  Alcotest.(check bool) "no envelope_id returns None" true
...
let test_extract_event_body () =
  ...
  Alcotest.(check string) "type" "event_callback" (json |> member "type" |> to_string);
  let event = json |> member "event" in
  Alcotest.(check string) "event type" "message" (event |> member "type" |> to_string);
...
let test_parse_envelope_empty_string_id () =
  ...
  Alcotest.(check bool) "empty envelope_id returns None" true
    (Slack_socket.parse_envelope msg = None)
...
let test_extract_event_body_non_object_payload () =
  ...
  let body = Slack_socket.extract_event_body payload in
  Alcotest.(check string) "fallback returns raw payload string"
    {|"raw-text"|} body
...
let suite =
  [
    ...
    Alcotest.test_case "parse envelope disconnect" `Quick test_parse_envelope_disconnect;
    Alcotest.test_case "parse envelope no id" `Quick test_parse_envelope_no_id;
    Alcotest.test_case "parse envelope invalid json" `Quick test_parse_envelope_invalid_json;
    Alcotest.test_case "extract event body" `Quick test_extract_event_body;
    Alcotest.test_case "extract event body reusable by slack" `Quick test_extract_event_body_reusable_by_slack;
    Alcotest.test_case "parse envelope empty string id" `Quick test_parse_envelope_empty_string_id;
    Alcotest.test_case "extract event body non-object payload" `Quick test_extract_event_body_non_object_payload;
    Alcotest.test_case "parse envelope empty payload" `Quick test_parse_envelope_empty_payload;
  ]
